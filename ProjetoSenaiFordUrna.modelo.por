...
programa
{
	...
	*/
	...
	funcao inicio()
	{
		...
		cadeia
			nomeCandidato1 = "Drauzio Varella",
			nomeCandidato2 = "Lula",
			nomeCandidato3 = "Pelé",
			nomeGanhador = ""
		...
		faca {
			...
			escolha(codigoVoto) {
				caso 021526:
				limpa()
				escreva(">> Deseja realemnte encerrar a vot~~ção?\n")
				escreva(">> Digite S para 'Sim': ")
				leia(confirmarEncerramento)
				
				se(confirmarEncerramento == 's' ou confirmarEncerramento == 'S') {
					encerraVotacao = verdadeiro
				}
					pare
				caso contrario: 
					opcaoInvalida = Verdadeira
